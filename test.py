...
from agents import AGENT_REGISTRY # Ensure this is correctly populated with your Tetris agents
...
def get_agent_file_prefix(agent_type_str, is_actor=False, is_critic=False):
    ...
    if agent_type_str == "genetic": return "genetic"
    if agent_type_str == "es": return "es"
    ...
    if agent_type_str == "ppo":
        ...
        for filename in os.listdir(model_dir):
            if filename.startswith(actor_prefix) and filename.endswith(".pth"):
                score_str = filename.replace(f"{actor_prefix}_score_", "").replace(".pth", "")
                try:
                    score = int(score_str)
                    ...
                    if os.path.exists(critic_full_path):
                        ...
                        if score not in potential_ppo_models or mtime > potential_ppo_models[score][2]:
                            potential_ppo_models[score] = (actor_full_path, critic_full_path, mtime)
...
def get_args():
    ...
    parser.add_argument("--agent_type", type=str, default="dqn",
                        choices=list(tetris_config.AGENT_TYPES), help="Type of agent to test.")
    parser.add_argument("--output_video_basename", type=str, default="best_tetris_game",
    ...
    parser.add_argument("--num_games", type=int, default=5, # Default to 5 games to find a best one
...
def play_game(env: Tetris, agent, game_seed, render_for_video=False, video_writer=None, max_pieces=10000):
    ...
    current_board_features = env.reset() # Tetris.reset() will use the current random state
    if tetris_config.DEVICE.type == "cuda":
    ...
    while not game_over and pieces_played < max_pieces:
        action_tuple, _ = agent.select_action(
            current_board_features, env, epsilon_override=0.0 # Always greedy for testing
        )
        
        # For video recording, pass the video_writer object to env.step's render
        reward, game_over = env.step(action_tuple, 
...
def test():
    ...
    networks_to_set_eval = ["policy_network", "qnetwork_local", "qnetwork_target", 
                            "actor", "critic", "network", "central_policy_net", "best_individual_network"]
    ...
    for i_game in range(opt.num_games):
        ...
        score, pieces, _, _ = play_game(env, agent, current_game_seed, 
                                        render_for_video=False, video_writer=None,
                                        max_pieces=tetris_config.MAX_PIECES_PER_EVAL_GAME)
        all_test_scores.append(score)
        print(f"  Preliminary Game {i_game + 1}/{opt.num_games}: Score={score}, Pieces={pieces}")
        if score > best_score_found:
            best_score_found = score
            seed_for_best_game = current_game_seed
            print(f"    New best score in this test batch: {best_score_found} (seed: {seed_for_best_game})")
    ...
    video_filename = f"{opt.output_video_basename}_score_{best_score_found}_{opt.agent_type}.mp4"
    video_path = os.path.join(tetris_config.MODEL_DIR, "test_videos", video_filename) # Save videos in a subfolder
    ...
    final_score, pieces, _, _ = play_game(env, agent, seed_for_best_game, 
                                          render_for_video=True, video_writer=out_video,
                                          max_pieces=tetris_config.MAX_PIECES_PER_EVAL_GAME)
