...
def get_agent_selection(
...
):  # HIER MUSS NOCH MAL REFACTORED WERDEN
    print(f"\n{prompt}")
    display_agents = (
    ...
    if allow_all and "all" not in options:
    ...
    while True:
        try:
            if allow_multiple:
                ...
                if "all" in user_input.lower().split(",") or "1" in user_input.split(
                    ","
                ):
                    user_input = "all"
...
def handle_training():
    ...
    for agent_type in agents_to_train:
        ...
        if agent_type in ["genetic", "es"]:
            ...
            if specific_num_gens_evolutionary:
            ...
            command.extend(["--agent_type", agent_type])
            if specific_num_epochs_dqn_reinforce:
            ...
            command.extend(["--agent_type", agent_type])
            if specific_total_steps_onpolicy:
