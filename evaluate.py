...
def find_latest_or_best_model_path(agent_type_str, model_dir):
    ...
    if not os.path.isdir(model_dir):
        print(f"Warning: Model directory {model_dir} does not exist for find_latest_or_best_model_path.")
...
def run_evaluation_game(env: Tetris, agent, game_seed_for_env_reset_unused):
    ...
    while not game_over and pieces_played_this_eval_run < tetris_config.MAX_PIECES_PER_EVAL_GAME:
        ...
        reward_for_step, game_over = env.step(action_tuple, render=should_render_this_step)
        ...
        if should_render_this_step and hasattr(env, 'render'): # Check if render is callable if it uses OpenCV
             if 'cv2' in sys.modules: # Only if cv2 is imported by tetris.py for rendering
...
def load_agent_for_eval(agent_type_to_load, state_size, model_base_dir):
    ...
    try:
        ...
        networks_to_eval = ["policy_network", "qnetwork_local", "qnetwork_target", 
                            "actor", "critic", "network", "central_policy_net",
        ...
        if agent_type_to_load == "genetic":
            ...
            if hasattr(agent_instance, 'get_best_policy_network'): 
                ...
                if best_ga_net:
                    ...
                    from agents.genetic_agent import GeneticAgent as GeneticAgentWrapper
                    return GeneticAgentWrapper(state_size, seed=tetris_config.SEED + 201) # Untrained wrapper
        ...
        return agent_class(state_size=state_size, seed=tetris_config.SEED + 201) # Return fresh instance
...
def main():
    ...
    if args.agent_types.lower() == "all":
        ...
        agent_types_to_evaluate = [agt for agt in tetris_config.AGENT_TYPES if agt.lower() in AGENT_REGISTRY]
    else:
    ...
    for agent_type_raw in agent_types_to_evaluate:
        ...
        agent_scores, agent_pieces_played, agent_tetrominoes, agent_lines_cleared = [], [], [], [] # Use distinct names
        ...
        for i_game in range(args.num_eval_games):
            ...
            score, pieces, tetrominoes_val, lines_val = run_evaluation_game(env, agent, 0) 
            agent_scores.append(score)
            agent_pieces_played.append(pieces) # Use the pieces played in this specific game
        ...
        stats = {
            ...
            "Avg Pieces": np.mean(agent_pieces_played) if agent_pieces_played else 0, # Use the correct list
    ...
    print("\n\n--- Evaluation Summary ---")
    headers = ["Agent", "Avg Score", "Std Score", "Min Score", "Max Score", 
               "Avg Pieces", "Avg Tetrominoes", "Avg Lines Cleared", "Num Eval Games"]
