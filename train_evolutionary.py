...
def train_genetic_algorithm(env_template: Tetris, cli_args_ns, model_base_dir: str):
    ...
    for gen in range(num_generations):
        mean_fitness, max_fitness_this_gen = ga_controller.evolve_population(
            env_template
        )
        ...
        print(
            f"GA Gen {gen + 1}/{num_generations} | Mean Pop Fit: {mean_fitness:.2f} | Max This Gen: {max_fitness_this_gen:.2f} | Controller Best Overall: {current_overall_best_fitness}"
        )

        if current_overall_best_fitness > highest_overall_fitness_on_disk:
            print(
                f"** New best overall GA fitness: {current_overall_best_fitness} (beats disk: {highest_overall_fitness_on_disk}). Saving model. **"
            )
            ...
            highest_overall_fitness_on_disk = (
                current_overall_best_fitness  # Update disk best
            )
    ...
    if final_best_fitness > highest_overall_fitness_on_disk:
        print(
            f"Final GA best fitness {final_best_fitness} is better than disk best {highest_overall_fitness_on_disk}. Saving."
        )
...
def train_evolutionary_strategies(
    env_template: Tetris, cli_args_ns, model_base_dir: str
):
...
if __name__ == "__main__":
    ...
    if "test_config" in str(tetris_config) and hasattr(
        tetris_config, "PROJECT_ROOT"
    ): 
        if (
            hasattr(tetris_config, "PROJECT_ROOT")
            and "models_test_suite" in tetris_config.MODEL_DIR
        ):  # if MODEL_DIR itself is the test dir
            current_model_base_dir = tetris_config.MODEL_DIR
        elif hasattr(
            tetris_config, "PROJECT_ROOT"
        ):  # Fallback to a standard test suite subfolder if not specified in MODEL_DIR
    ...
    if cli_args.agent_type == "genetic":
        ...
        print(
            f"Error: Agent type '{cli_args.agent_type}' is not supported by this evolutionary training script."
        )
