...
class GeneticAlgorithmController:
    ...
    def _initialize_population(self):
        ...
        for i in range(self.population_size):
            ...
            policy_net = PolicyNetwork(self.state_size, self.action_size, seed=individual_seed).to(DEVICE)
    ...
    def _evaluate_fitness(self, individual_policy_net, eval_env_template: Tetris):
        ...
        for _ in range(self.eval_games_per_individual):
            ...
            while not game_over and pieces_this_game < self.max_pieces_per_eval_game :
                ...
                if not next_steps_dict: break # No moves possible
                ...
                with torch.no_grad():
                    action_scores = individual_policy_net(feature_tensors_for_next_steps)
    ...
    def _crossover(self, parent1_net: PolicyNetwork, parent2_net: PolicyNetwork):
        child1_net = PolicyNetwork(self.state_size, self.action_size, seed=random.randint(0,1000000)).to(DEVICE)
        child2_net = PolicyNetwork(self.state_size, self.action_size, seed=random.randint(0,1000000)).to(DEVICE)
        ...
        if random.random() < self.crossover_rate:
            ...
            child2_w = (p1_weights + p2_weights) / 2.0 # Can be same or one parent's clone
    ...
    def evolve_population(self, eval_env_template: Tetris):
        ...
        while offspring_generated < num_offspring_needed:
            ...
            p2 = parents[(parent_idx + 1 + random.randint(0,len(parents)-2)) % len(parents)] # Ensure p2 is different
            parent_idx += 1
    ...
    def load_best_individual(self, filename=None, state_size_override=None):
        path = filename if filename else global_config.GA_MODEL_PATH
        _state_size = state_size_override if state_size_override is not None else self.state_size
        if not _state_size:
...
class GeneticAgent(BaseAgent):
    ...
    def __init__(self, state_size, seed=0, policy_network_instance: PolicyNetwork = None):
        ...
        if self.policy_network is None:
            ...
            self.policy_network = PolicyNetwork(state_size, action_size=1, seed=seed).to(DEVICE)
    ...
    def load(self, filepath=None): # Loads into this agent instance's policy network
        ...
        if os.path.exists(path):
            ...
            if self.policy_network is None:
                self.policy_network = PolicyNetwork(self.state_size, action_size=1, seed=0).to(DEVICE)
